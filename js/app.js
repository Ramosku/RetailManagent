...
function renderAll(){
  ...
  renderPersonal(); renderHorario(); renderAsistencia(); renderEvaluaciones();
  renderFallos(); renderAuditHistory();
  buildLimpieza(); cargarFechasHistorialLim();
  document.getElementById('sip-fecha').value = new Date().toISOString().split('T')[0];
  renderSipHistory();
}
...
document.querySelectorAll('.nav-item').forEach(el=>{
  el.addEventListener('click',()=>{
    ...
    const titles={dashboard:'Dashboard general',vencimientos:'Control de vencimientos',incidencias:'Incidencias de recepción',limpieza:'Limpieza',auditorias:'Auditorías de Tienda',sip:'SIP',personal:'Mi equipo',horarios:'Horarios semanales',asistencia:'Asistencia',evaluaciones:'Evaluaciones'};
