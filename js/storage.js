...
function loadDB(){
  ...
  return {
    ...
    nextId: {venc:1, inc:1, eval:1, pers:1, aud:1, fallo:1, caida:1, cert:1}
  };
}

// Campos de versiones antiguas de la app que ya no se usan. Si quedan
// colgados en localStorage o en Firebase, se eliminan en cada carga para
// que no se sigan subiendo a la nube al guardar.
const LEGACY_FIELDS = ['sip','venc','inc','eval','pers','aud','fallo','caida'];
...
function migrateDB(parsed){
  LEGACY_FIELDS.forEach(f=>{ delete parsed[f]; });
  ...
  if(!parsed.nextId.cert) parsed.nextId.cert = 1;
