// ── UTILS ────────────────────────────────────────────
// ── IDs ──────────────────────────────────────────────
// El id se calcula según lo que realmente existe en cada tabla, no con
// un contador que solo sube. Si la tabla está vacía, el próximo es el 1.
const UID_ARRAYS = { venc:'vencimientos', inc:'incidencias', eval:'evaluaciones', pers:'personal', caida:'caidas', cert:'certificados' };
function uid(type){
  const arrName = UID_ARRAYS[type];
  const arr = (arrName && Array.isArray(DB[arrName])) ? DB[arrName] : [];
  const nums = arr.map(x=>typeof x.id==='number'?x.id:parseInt(x.id)).filter(n=>!isNaN(n));
