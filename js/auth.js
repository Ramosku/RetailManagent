// ── TIENDAS ──────────────────────────────────────────
// Login compartido por tienda (no por empleado). Cada tienda tiene
// su propia cuenta de Firebase y sus propios datos, totalmente separados.
const TIENDAS = [
...
async function doLogin(){
  const t = document.getElementById('login-user').value;
  const p = document.getElementById('login-pass').value;
  const err = document.getElementById('login-error');
  const btn = document.querySelector('.btn-login');
  if(!t){ err.style.display='block'; err.textContent='Selecciona una tienda.'; return; }
  ...
  try{
    ...
    err.textContent='Tienda o contraseña incorrectos.';
  }finally{
